...
Entrada:  validaciones_salidas_{STATION_ID}_pred.csv  (salida del modelo)
          *-salidas.parquet                           (datos reales)
...
for archivo in archivos:
    ...
    if df_est.empty:
        continue
    partes.append(df_est[["datetime", "salidas"]])
...
if hay_real_pred:
    ...
    if len(comp) > 0:
        mae  = mean_absolute_error(comp["salidas"], comp["prediccion"])
        rmse = mean_squared_error(comp["salidas"], comp["prediccion"]) ** 0.5
        mape = (np.abs(comp["salidas"] - comp["prediccion"]) /
                comp["salidas"].replace(0, np.nan)).mean() * 100
...
if len(real_2025) > 0:
    perfil_real25 = real_2025.groupby(real_2025["datetime"].dt.hour)["salidas"].mean()
else:
    perfil_real25 = None

# Perfil real del período predicho si existe
if hay_real_pred and len(comp) > 0:
    perfil_real_pred = comp.groupby(comp["datetime"].dt.hour)["salidas"].mean()
else:
...
fig.suptitle(
    f"Estación {STATION_ID}  —  Predicción de salidas 2026 (XGBoost)",
    fontsize=13, y=1.01
)
...
if hay_real_pred:
    ax.plot(comp["datetime"], comp["salidas"],
...
if hay_real_pred:
    ...
    ax.plot(comp.loc[mask_2s_real, "datetime"], comp.loc[mask_2s_real, "salidas"],
...
#ax.set_ylabel("Promedio de salidas")
...
#    ax = axes[3]
#    errores = comp["prediccion"] - comp["salidas"]
#
