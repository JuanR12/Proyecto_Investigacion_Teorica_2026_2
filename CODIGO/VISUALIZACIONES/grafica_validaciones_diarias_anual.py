...
from config import RUTA_PARQUET as RUTA, RUTA_FIGURAS

def cargar_serie(tipo):
    archivos = sorted(glob.glob(str(RUTA / f"*-{tipo}.parquet")))
    partes = []
    for f in archivos:
...
plt.tight_layout()
plt.savefig(RUTA_FIGURAS / "serie_anual_transmilenio.png", dpi=150, bbox_inches="tight")
plt.show()
print(f"Guardado en {RUTA_FIGURAS / 'serie_anual_transmilenio.png'}")